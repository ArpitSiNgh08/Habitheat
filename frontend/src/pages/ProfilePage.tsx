...
const ProfilePage: React.FC<ProfilePageProps> = ({ theme }) => {
  const [activeTab, setActiveTab] = useState<"achievements" | "mood">(
    "achievements"
  );
  const storedUser = localStorage.getItem("user");
  const initialProfilePic =
    storedUser && JSON.parse(storedUser).profilePic
      ? JSON.parse(storedUser).profilePic
    ...
    fetch("http://localhost:5000/api/auth/me", {
      ...
      .then((data) => {
        ...
        setUserProfile((prev) => ({
          ...
          profilePic: data.profilePic || prev.profilePic, // <-- Use avatar from backend
        }));
      })
      .catch(() => {
      ...
      try {
        ...
        if (storedUser) {
          ...
          setUserProfile((prev) => ({
            ...
            profilePic: userData.profilePic || prev.profilePic, // <-- add this line
          }));
        }
      } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 p-4 md:p-8 transition-colors">
      <div className="max-w-7xl mx-auto">
        ...
        <div
          className={
            isEditingProfile
              ? "grid grid-cols-1 md:grid-cols-[35%_65%] gap-4"
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 sticky top-8 border border-gray-200 dark:border-gray-700">
              ...
              <div className="text-center mb-8">
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
                  {userProfile.name || authUser?.username || "John Doe"}
                </h2>
                <p className="text-gray-600 dark:text-gray-300 mb-1">
                  {userProfile.email || authUser?.email || "john.doe@example.com"}
                </p>
            ...
            <div className="flex flex-col">
              <div className="flex sm:gap-20 gap-10">
                <ArrowLeft
                  ...
                  className="dark:text-white ml-10 cursor-pointer"
                />
                <h1 className="dark:text-white text-2xl font-bold lg:ml-36 md:ml-8 sm:ml-11 text-center">
                  Change your details
                </h1>
              </div>
              <form className="m-8" onSubmit={handleSubmit}>
                ...
                <input
                  ...
                  placeholder={authUser?.username || userProfile.name || "John Doe"}
                  onChange={(e) => setUsername(e.target.value)}
                />
                <label className="dark:text-white m-2">Email:</label>
                <input
                  type="email"
                  value={authUser?.email || userProfile.email || ""}
                  disabled
                ...
                <div className="flex items-center justify-between mt-5 ml-2 mb-3">
                  <label className="flex items-center cursor-pointer">
                    {/* Toggle Label */}
                    <span className="mr-3 text-sm dark:text-white text-gray-700">
                      Change Password?
                    </span>
                ...
                <h1 className="dark:text-white ml-2 text-xl my-4 font-bold">
                  Profile Settings
                </h1>
                <TimeInput
                ...
                <label
                  htmlFor="week"
                  className="text-sm ml-2 text-gray-700 dark:text-white mb-1 block"
                >
                ...
                <label
                  htmlFor="week"
                  className="text-sm ml-2 text-gray-700 dark:text-white mb-1 mt-2 block"
                >
                  What is your age?
                </label>
                <input
